//! TUI 的集中式运动原语。
//!
//! 调用方在此选择显式的减少运动回退，而不是
//! 直接求助于随时间变化的 spinner 或 shimmer 辅助函数。
...
use crate::tui_core::shimmer::shimmer_spans;
...
mod tests {
    ...
    fn animation_primitives_are_only_used_by_motion_module() {
        ...
        let lib_rs = crate::utils_cargo_bin::find_resource!("src/lib.rs")
            ...
            for (line_number, line) in contents.lines().enumerate() {
                ...
                if direct_spinner.is_match(code) {
                    violations.push(format!(
                        "{relative_path}:{} contains a direct `spinner(...)` call; use crate::tui_core::motion instead",
                        line_number + 1
                    ));
                }
                if direct_shimmer.is_match(code) {
                    violations.push(format!(
                        "{relative_path}:{} contains a direct `shimmer_spans(...)` call; use crate::tui_core::motion instead",
                        line_number + 1
                    ));
                }
            }
