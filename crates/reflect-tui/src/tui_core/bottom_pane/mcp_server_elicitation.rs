//! MCP 服务端 elicitation 覆盖层（form 审批 / schema 表单）。
//!
//! 本文件持有 `McpServerElicitationOverlay` 的核心状态机与 `Renderable`/`BottomPaneView`
//! 实现。schema 解析与工具审批展示参数构建已外移至 [`schema`] 子模块。
//!
//! 注：本文件超过 800 行红线，但 `impl McpServerElicitationOverlay` 为内聚的完整状态机
//! （与 上游 `bottom_pane/mcp_server_elicitation.rs` 逐行对应），按 AGENTS.md
//! 「内聚完整状态机例外」保留，不强制按方法簇切分，以降低与上游的 re-sync 分歧。

use std::collections::HashSet;
use std::collections::VecDeque;
use std::path::PathBuf;

use crate::app_server_protocol::McpElicitationEnumSchema;
use crate::app_server_protocol::McpElicitationPrimitiveSchema;
use crate::app_server_protocol::McpElicitationSingleSelectEnumSchema;
...
use crate::protocol_compat::mcp_approval_meta::APPROVAL_KIND_KEY as APPROVAL_META_KIND_KEY;
use crate::protocol_compat::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as APPROVAL_META_KIND_MCP_TOOL_CALL;
use crate::protocol_compat::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as APPROVAL_META_KIND_TOOL_SUGGESTION;
use crate::protocol_compat::mcp_approval_meta::PERSIST_ALWAYS as APPROVAL_PERSIST_ALWAYS_VALUE;
use crate::protocol_compat::mcp_approval_meta::PERSIST_KEY as APPROVAL_PERSIST_KEY;
use crate::protocol_compat::mcp_approval_meta::PERSIST_SESSION as APPROVAL_PERSIST_SESSION_VALUE;
use crate::protocol_compat::mcp_approval_meta::TOOL_NAME_KEY;
use crate::protocol_compat::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use crate::protocol_compat::mcp_approval_meta::TOOL_PARAMS_KEY as APPROVAL_TOOL_PARAMS_KEY;
...
use crate::tui_core::bottom_pane::selection_popup_common::menu_surface_inset;
use crate::tui_core::bottom_pane::selection_popup_common::menu_surface_padding_height;
use crate::tui_core::bottom_pane::selection_popup_common::render_menu_surface;
use crate::tui_core::bottom_pane::selection_popup_common::render_rows;
...
use crate::tui_core::text_formatting::format_json_compact;
