...
use crate::tui_core::bottom_pane::command_popup::CommandItem;
...
use crate::tui_core::bottom_pane::slash_commands::has_slash_command_prefix;
...
impl ChatComposer {
            ...
            KeyEvent {
                    ...
                    && let Some(sel) = popup.selected_item()
                {
                   if self.blocks_direct_input {
                    ...
                    {
                        // 命令(带 inline args)已分发 —— args 已 captured 进
                        // `CommandWithArgs`,清空 textarea,与 bare 命令 / 提交路径
                        // 对齐。否则 `/plan <task>` `/review <args>` 在弹窗打开时提交
                        // 会让原文本残留在输入框,看起来「没反应」。
                        self.draft.textarea.set_text_clearing_elements("");
                        self.draft.is_bash_mode = false;
                        return (result, true);
    ...
    fn complete_selected_slash_command_preserving_existing_draft_tail_as_inline_args(
        ...
        // 始终只替换「已输入的命令 token」(text[0..command_token_end])，保留其
        // 后的全部文本作为 inline args。此前用 `cursor` 作 replace_end 是 bug：
        // 当光标不在命令 token 内部（典型场景：已输入完整命令 + 参数、光标停在
        // 末尾或参数区）时，会把命令名尾部（如 "/loop" 的 "oop"）误当 tail 保留，
        // 前面再补 "/loop "，拼成 "/loop oop 3 /status"，参数首段被解析成非法
        // interval 'oop'。args 的语义是「命令 token 之后的全部文本」，与光标
        // 位置无关 —— 故 replace_end 固定为 command_token_end。
        let replace_end = command_token_end;
...
/// 将全文元素范围转换为命令参数范围。
///
/// `rest_offset` 是 `rest` 在全文中起始位置的字节偏移量。
...
mod tests {
    ...
    fn slash_completion_preserves_existing_draft_tail_for_inline_arg_commands() {
        // inline args = 命令 token（首个空白之前）之后的全部文本，与光标位置无关。
        // 回归：此前用 `cursor` 切割 tail，当光标不在命令 token 内部（典型：已输入
        // 完整命令 + 参数、光标停在末尾）时会吞掉整段参数，或把命令名后缀误当
        // args —— 实战中 `/loop 3 /status` 被解析成 interval 'oop'、
        // `/mode accept-edits` 变成 'ccept-edits'。这里覆盖两类场景。

        // 场景 1：完整命令 + 参数，光标在末尾（最常见、回归核心）。
        let text1 = "/review the diff";
        let mut composer = composer_with_text_at_cursor(text1, text1.len());
        assert_eq!(
            press(&mut composer, KeyCode::Enter),
            InputResult::CommandWithArgs(SlashCommand::Review, "the diff".to_string(), Vec::new())
        );
        // Enter 已分发命令（args captured 进 CommandWithArgs），textarea 清空。
        assert_eq!(composer.draft.textarea.text(), "");

        // 场景 2：光标停在命令 token 中部，命令 token 之后的参数仍应完整保留。
        let mut composer = composer_with_text_at_cursor("/review the diff", "/re".len());
        assert_eq!(
            press(&mut composer, KeyCode::Enter),
            InputResult::CommandWithArgs(SlashCommand::Review, "the diff".to_string(), Vec::new())
        );
