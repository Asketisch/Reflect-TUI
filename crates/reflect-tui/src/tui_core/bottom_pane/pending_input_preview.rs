...
/// 显示回合进行中持有的待处理转向与后续输入的 widget。
///
/// 该 widget 先渲染待处理转向，然后是回合结束时将
/// 重新提交的被拒绝转向，最后是普通排队用户消息。待处理
/// 转向会说明它们将在下一个工具/结果边界后提交，除非
/// 用户调用中断绑定立即发送。底部的编辑提示仅在有实际
/// 排队用户输入可弹回 composer 时出现。由于某些终端
/// 会拦截特定的修饰键组合，显示的绑定可通过
/// [`set_edit_binding`](Self::set_edit_binding) 配置。
pub(crate) struct PendingInputPreview {
    pub pending_steers: Vec<String>,
    pub rejected_steers: Vec<String>,
    pub queued_messages: Vec<String>,
    /// 提示行中渲染的按键组合。默认为 Alt+Up，但
    /// 可在该组合键不可用的终端上覆盖。
