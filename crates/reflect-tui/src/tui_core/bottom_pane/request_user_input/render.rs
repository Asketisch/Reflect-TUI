...
use crate::tui_core::bottom_pane::selection_popup_common::wrap_styled_line;
...
struct UnansweredConfirmationData {
    ...
    rows: Vec<crate::tui_core::bottom_pane::selection_popup_common::GenericDisplayRow>,
    state: ScrollState,
}
...
impl Renderable for RequestUserInputOverlay {
    fn desired_height(&self, width: u16) -> u16 {
        ...
        // 备注可见时，输入框本身已将选项与页脚隔开。
        // 没有备注时，我们保留额外间距，避免页脚提示紧贴选项。
        ...
        let mut height = question_height
            ...
            .saturating_add(PROGRESS_ROW_HEIGHT); // 进度行
...
/// 将行渲染到 `area` 中，当生成的行数少于 `area.height` 时，
/// 可见的行会底部对齐。
///
/// 通过将选项块锚定在其分配区域的底部，保持页脚间距稳定。
fn render_rows_bottom_aligned(
    area: Rect,
    buf: &mut Buffer,
    rows: &[crate::tui_core::bottom_pane::selection_popup_common::GenericDisplayRow],
    state: &ScrollState,
    max_results: usize,
    empty_message: &str,
) {
...
/// 将带样式的行截断到 `max_width`，优先在单词边界处截断，并追加省略号。
///
/// 此函数逐字符遍历各 span，同时记录可用宽度内（不含省略号宽度）最后一个
/// 宽度安全的位置和最后一个空白边界。若行溢出，则尽可能在最后一个单词边界
/// 截断（无法满足时回退到最后一个可容纳的字符），去除尾部空白，然后追加
/// 与最后一个可见 span 样式一致的省略号（若未保留任何内容则使用行样式）。
