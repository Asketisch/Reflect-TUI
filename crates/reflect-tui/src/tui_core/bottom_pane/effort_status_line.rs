//! 当推理投入档位切换为 Max 或 Ultra 时的一次性状态行过渡动画。
//!
//! 旧状态行在逐渐带上档位强调色并向右滑动后淡出；随后档位字母带更宽的
//! 边侧间距铺满整行、汇聚成居中的 `M A X` 或 `U L T R A`，并在刷新后的
//! 状态行淡入之前淡出。时钟只在被动页脚行被渲染时启动，因此选择器、闪屏或
//! 说明性页脚不会在动画可见之前消耗它。ANSI-16 与颜色类型未知的终端会
//! 跳过过渡、保留普通状态行；浏览器后端以及支持 ANSI-256 或 truecolor 的
//! 原生终端可看到完整效果。
...
use crate::tui_core::line_truncation::truncate_line_to_width;
