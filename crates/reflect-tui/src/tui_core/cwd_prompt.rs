...
use crate::tui_core::legacy_core::config::edit::ConfigEditsBuilder;
...
use crate::tui_core::render::renderable::RenderableExt as _;
use crate::tui_core::selection_list::selection_option_row;
...
pub(crate) async fn run_cwd_selection_prompt(
    ...
    if screen.should_exit {
        ...
        if let Some(error_line) = persist_remembered_cwd_selection(config, selection).await {
            tui.insert_history_lines(vec![error_line.to_string()]);
        }
...
mod tests {
    ...
    async fn cwd_prompt_remembered_choices_select_and_persist_matching_directory() -> Result<()> {
        ...
        let config = ConfigBuilder::default()
            .reflect_home(temp_dir.path().to_path_buf())
            .build()
            .await?;
