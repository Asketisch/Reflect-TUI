//! `ChatWidget` 的 Windows 沙箱提示与警告界面。
...
impl ChatWidget {
    ...
    pub(super) fn elevated_windows_sandbox_setup_required(&self) -> bool {
        crate::tui_core::windows_sandbox::level_from_config(&self.config)
            ...
            && self
                ...
                .source
                .is_some()
            && !crate::tui_core::windows_sandbox::sandbox_setup_is_complete(
                self.config.reflect_home.as_path(),
            )
    }
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        let Ok(permissions) =
            crate::tui_core::windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                &permission_profile,
        ...
        match crate::tui_core::windows_sandbox::apply_world_writable_scan_and_denies_for_permissions(
            self.config.reflect_home.as_path(),
            cwd.as_path(),
            &env_map,
            &permissions,
            Some(self.config.reflect_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_world_writable_warning_confirmation(
        ...
        // 构建确保确认先于应用新权限配置发生的动作，
        // 使下游的策略变更钩子不会
        // 重新触发该警告。
        let mut accept_actions: Vec<SelectionAction> = Vec::new();
        // 只有在通过 /permissions 应用预置时才抑制立即重新扫描，
        // 避免随后的策略变更产生重复警告。
        if preset.is_some() {
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        self.session_telemetry.counter(
            "reflect.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the Reflect agent sandbox to protect your files and control network access."],
                ]
            } else {
                vec![
                    line!["Your organization requires the default Reflect agent sandbox to continue. Set it up to protect your files and control network access."],
                ]
            })
        ...
        let mut items = vec![SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                accept_otel.counter(
                    "reflect.windows_sandbox.elevated_prompt_accept",
                    /*inc*/ 1,
                    &[],
                );
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "reflect.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                quit_otel.counter(
                    "reflect.windows_sandbox.elevated_prompt_quit",
                    /*inc*/ 1,
                    &[],
                );
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Reflect in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before Reflect can continue."
            ]);
        }
        ...
        let mut items = vec![SelectionItem {
            ...
            actions: vec![Box::new({
                ...
                move |tx| {
                    otel.counter(
                        "reflect.windows_sandbox.fallback_retry_elevated",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Reflect with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "reflect.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                quit_otel.counter(
                    "reflect.windows_sandbox.fallback_prompt_quit",
                    /*inc*/ 1,
                    &[],
                );
                tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
