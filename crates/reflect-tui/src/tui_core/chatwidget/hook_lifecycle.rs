//! `ChatWidget` 的钩子运行生命周期处理。
//!
//! 本模块把活跃钩子单元格、钩子计时器与钩子完成输出集中在一起维护。
...
impl ChatWidget {
    ...
    pub(super) fn on_hook_started(&mut self, run: crate::app_server_protocol::HookRunSummary) {
    ...
    pub(super) fn on_hook_completed(
        &mut self,
        completed: crate::app_server_protocol::HookRunSummary,
    ) {
