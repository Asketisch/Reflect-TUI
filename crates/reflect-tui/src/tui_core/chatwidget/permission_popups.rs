//! `ChatWidget` 的权限与审批弹窗流程。
//!
//! 本模块负责通用的权限选择器与确认界面；
//! Windows 专用的沙箱提示位于相邻的 `windows_sandbox_prompts`。
...
impl ChatWidget {
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        let guardian_disabled_reason = |enabled: bool| {
            ...
            let _ = next_features.set_enabled(Feature::GuardianApproval, enabled);
            if self.config.features.can_set(&next_features).is_err() {
                Some("Cannot set guardian approval feature.".to_string())
            } else {
                None
            }
        };
        ...
        for preset in presets.into_iter() {
                ...
                .can_set(&preset.approval)
            {
                ...
                Some("Cannot set this approval policy".to_string())
            };
    ...
    pub(super) fn permission_mode_actions(
        ...
        if approvals_reviewer == ApprovalsReviewer::User && preset.id == "auto" {
            #[cfg(target_os = "windows")]
            {
                if crate::tui_core::windows_sandbox::level_from_config(&self.config)
                    == WindowsSandboxLevel::Disabled
                {
                    let preset = preset.clone();
                    if crate::tui_core::windows_sandbox::sandbox_setup_is_complete(
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When Reflect runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
