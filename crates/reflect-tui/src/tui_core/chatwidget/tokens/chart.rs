//! 为 `/usage` 渲染账户令牌用量摘要和活动图表。
//!
//! 本模块负责图表数据分桶和 ratatui 行构建。异步卡片生命周期保留在父模块
//! `tokens` 中，使图表渲染保持为从已加载的用量响应出发的纯转换。
...
impl TokenActivityView {
    /// 将可选的 `/usage` 参数解析为受支持的图表视图。
    ///
    /// 空参数会选择每日视图，因此 `/usage` 和 `/usage daily` 行为一致。
    /// 返回 `None` 可让斜杠命令分发器报告不支持的参数，而不是默默选择某个视图。
    pub(in crate::tui_core::chatwidget) fn parse(value: &str) -> Option<Self> {
...
fn chart_lines(
    view: TokenActivityView,
    buckets: &[crate::app_server_protocol::AccountTokenUsageDailyBucket],
    today: NaiveDate,
    width: u16,
) -> Vec<Line<'static>> {
...
/// 以贪心方式将摘要字段打包成尽量少、且能放进 `width` 的行，
/// 并保持字段顺序。`u16::MAX`（raw/copy 模式）总是生成一行。
...
/// 将当前连续天数与最长连续天数合并为一个字段：两者相同时只显示 `54d`，
/// 否则显示 `12d (best 54d)`。
...
fn weekday_label(view: TokenActivityView, row: usize) -> Span<'static> {
    if view != TokenActivityView::Daily {
        // 柱状视图从底部（第 6 行）向上填充，因此侧栏兼任粗略的 Y 轴：
        // 顶部为峰值，底部为基线。
        return Span::styled(
            match row {
                0 => "max ",
...
/// 柱状图视图的说明文字，因为 5 级的每日图例在这里会产生误导。
/// 说明每根柱代表的含义，以及缩放所依据的峰值。
...
/// 将后端的每日分桶归一化到固定的 52 周显示窗口。
///
/// 返回的向量按图表单元格排序，从最早的周日开始。无效、超出窗口以及
/// 未来的日期会被忽略。重复日期会被累加，负的令牌数值不会减少活动量。
fn daily_values(
    buckets: &[crate::app_server_protocol::AccountTokenUsageDailyBucket],
    today: NaiveDate,
) -> Vec<i64> {
