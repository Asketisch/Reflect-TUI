//! `ChatWidget` 的模型、协作模式和推理弹窗。
//!
//! 这些界面联系紧密，因为更改其中之一经常会跳转到另一个界面，
//! 尤其是在 Plan 模式处于活跃状态时。
...
impl ChatWidget {
    /// 打开用于选择快速 auto 模型的弹窗。选择“所有模型”会打开包含所有可用预设的完整选择器。
    ...
    fn custom_openai_base_url(&self) -> Option<String> {
        ...
        let base_url: &str = self.config.model_provider.base_url.as_ref();
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("reflect-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "reflect-auto-fast" => 0,
            "reflect-auto-balanced" => 1,
            "reflect-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running reflect -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    fn should_prompt_plan_mode_reasoning_scope(
        ...
        // 只要当前选择对以下两者而言都不是真正的“空操作”，就弹出提示：
        // 1) 当前 Plan 模式下实际生效的推理设置；
        // 2) 回退路径会修改的、已存储的全局默认值。
    ...
    /// 打开为指定模型选择标准推理强度的弹窗。
    ///
    /// Max 和 Ultra 需要明确的第二步操作，这样在浏览常规强度级别时
    /// 不会意外选择到昂贵的强度。
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-pro")
            || preset.model.starts_with("gpt-5.1-pro-max")
