...
impl ChatWidget {
    ...
    /// 处理因用户中断（Esc）、预算耗尽
    /// 或复盘完成而中止的回合。
    /// 存在排队用户消息时，将它们以换行分隔恢复到 composer，
    /// 而不是自动提交下一条。
    pub(super) fn on_interrupted_turn(&mut self, reason: TurnAbortReason) {
        ...
        // 中断的回合到达 UI 时，服务器已丢弃待处理的输入，因此
        // 任何此处仍跟踪的未确认转向都必须本地恢复，
        // 而不是等待后续提交。
        if send_pending_steers_immediately {
    ...
    /// 把待处理转向、排队草稿与当前 composer 状态合并为一条消息。
    ///
    /// 每条待处理消息从 `[Image #1]` 开始相对自身的远程
    /// 图片编号附件。中断后拼接多条消息时，必须
    /// 以稳定顺序重新编号本地图片
    /// 占位符并重置文本元素字节范围，使恢复的 composer
    /// 状态与合并后的附件列表保持一致。没有可恢复内容时返回 `None`。
