...
impl ChatWidget {
            ...
            ServerNotification::Error(notification) => {
                if notification.will_retry {
                    ...
                    self.handle_non_retry_error(
                        notification.error.message,
                        notification.error.reflect_error_info,
                    );
                }
            }
    ...
    pub(super) fn handle_turn_completed_notification(
        ...
        // 用户消息去重只抑制本 TUI 已在本地渲染的提示的 app 服务端回声。
        // 一旦该回合结束，另一个客户端提交相同的文本时，
        // 它仍需要属于自己的用户单元格。
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                        ...
                        == Some(&(notification.turn.id.clone(), error.message.clone()))
                    {
                        ...
                        self.handle_non_retry_error(error.message, error.reflect_error_info);
                    }
                } else {
                    self.last_non_retry_error = None;
                    self.finalize_turn();
