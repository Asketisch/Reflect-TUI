//! `ChatWidget` 的交互式工具请求界面。
//!
//! 本模块负责会阻塞等待用户决策的审批、权限、征询与用户输入等提示。
...
impl ChatWidget {
    ...
    /// 处理当前会话线程的守护审查生命周期事件。
    ///
    /// 进行中的评估会临时占用实时状态页脚，让用户能看到正在审查的内容，
    /// 包括并行审查的聚合情况。终结评估会清除或更新该页脚状态，
    /// 并在守护返回决定后渲染最终的通过/拒绝历史单元格。
    pub(super) fn on_guardian_assessment(&mut self, ev: GuardianAssessmentEvent) {
        ...
        {
            // 进行中的评估在审查期间占用实时页脚状态。
            // 并行审查由 `PendingGuardianReviewStatus` 聚合成一条页脚摘要。
        ...
        if ev.status == GuardianAssessmentStatus::Approved {
            let cell = if let Some(command) = guardian_command(&ev.action) {
                history_cell::new_approval_decision_cell(
                    ...
                    crate::tui_core::history_cell::ReviewDecision::Approved,
        ...
        if ev.status == GuardianAssessmentStatus::TimedOut {
            let cell = if let Some(command) = guardian_command(&ev.action) {
                history_cell::new_approval_decision_cell(
                    history_cell::ApprovalDecisionSubject::Command(command),
                    crate::tui_core::history_cell::ReviewDecision::TimedOut,
                    history_cell::ApprovalDecisionActor::Guardian,
                )
            } else {
                match &ev.action {
                    ...
                    GuardianAssessmentAction::McpToolCall {
                        ...
                        "reflect could call MCP tool {server}.{tool_name}"
                    )),
                    GuardianAssessmentAction::NetworkAccess { target, .. } => {
                        history_cell::new_guardian_timed_out_action_request(format!(
                            "reflect could access {target}"
                        ))
                    }
                    GuardianAssessmentAction::RequestPermissions { reason, .. } => {
                        history_cell::new_guardian_timed_out_action_request(
                            permission_request_summary("reflect could request permissions", reason),
                        )
                    }
        ...
        let cell = if let Some(command) = guardian_command(&ev.action) {
            history_cell::new_approval_decision_cell(
                history_cell::ApprovalDecisionSubject::Command(command),
                crate::tui_core::history_cell::ReviewDecision::Denied,
                history_cell::ApprovalDecisionActor::Guardian,
            )
        } else {
            match &ev.action {
                ...
                GuardianAssessmentAction::McpToolCall {
                    ...
                    "reflect to call MCP tool {server}.{tool_name}"
                )),
                GuardianAssessmentAction::NetworkAccess { target, .. } => {
                    history_cell::new_guardian_denied_action_request(format!(
                        "reflect to access {target}"
                    ))
                }
                GuardianAssessmentAction::RequestPermissions { reason, .. } => {
                    history_cell::new_guardian_denied_action_request(permission_request_summary(
                        "reflect to request permissions",
                        reason,
                    ))
                }
                GuardianAssessmentAction::Command { .. } => unreachable!(),
    ...
    pub(crate) fn handle_exec_approval_now(&mut self, ev: ExecApprovalRequestEvent) {
        ...
        self.bottom_pane
            .push_approval_request(request, &Features::from(&self.config.features));
        self.set_ambient_pet_notification(
            crate::tui_core::pets::PetNotificationKind::Waiting,
            /*body*/ None,
        );
        self.request_redraw();
    }
    ...
    pub(crate) fn handle_elicitation_request_now(
        ...
        if let Some(params) =
            crate::tui_core::bottom_pane::AppLinkViewParams::from_url_app_server_request(
                thread_id,
                &params.server_name,
                request_id.clone(),
                &params.request,
            ...
            match params.request {
                ...
                | McpServerElicitationRequest::Url { .. } => {
                    self.app_event_tx.resolve_elicitation(
                        ...
                        params.server_name,
                        request_id,
                        crate::app_server_protocol::McpServerElicitationAction::Decline,
                        /*content*/ None,
                        /*meta*/ None,
                    );
                }
