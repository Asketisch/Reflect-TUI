//! `ChatWidget` 的用户消息与 shell 提示提交行为。
...
impl ChatWidget {
    ...
    fn submit_user_message_with_history_and_shell_escape_policy(
        ...
        // 特例："!cmd" 在本地执行 shell 命令，而不是发送给模型。
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = crate::connectors::metadata::connector_mention_slug(&app);
        ...
        // App 事件提交是串行处理的，turn/start 可能要等待远端工作。
        // 先入队乐观提示，让用户的输入在此期间能够显示出来。
        // 直接提交不共享该队列，因此保留其原有的失败行为。
        let render_before_submit =
            render_in_history && matches!(&self.reflect_op_target, ReflectOpTarget::AppEvent);
        ...
        // 将提交的文本持久化到跨会话的消息历史中。提及会被编码为占位符语法，
        // 以便在未来的会话中回溯时能够重建提及绑定。
    ...
    /// 恢复被阻塞的提交草稿，且不丢失提及解析状态。
    ///
    /// 图片被阻塞的路径刻意将草稿保留在输入框中，方便用户移除附件后重试。
    /// 我们必须连同可见文本一起恢复提及绑定；只恢复 `$name` token 会让草稿
    /// 看起来正常，但重试时提及解析将退化为仅按名称的启发式。
