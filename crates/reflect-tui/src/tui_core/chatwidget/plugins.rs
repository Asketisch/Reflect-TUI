//! 插件安装/卸载/启用生命周期。
//!
//! 注：本文件超过 800 行红线，但 `impl ChatWidget` 插件生命周期方法簇为内聚的完整
//! 状态机（31 个方法），与上游逐行对应，按 AGENTS.md「内聚完整状态机例外」保留。
...
impl ChatWidget {
    ...
    fn plugin_install_auth_popup_params(&self) -> Option<SelectionViewParams> {
        ...
        let status_label = if is_installed {
            ...
            "Install the required Apps in Reflect to continue:"
        };
        ...
        if let Some(install_url) = app.install_url.clone() {
            ...
            items.push(SelectionItem {
                ...
                description: Some("Open the Reflect app management page".to_string()),
            ...
            items.push(SelectionItem {
                name: "Reflect apps link unavailable".to_string(),
