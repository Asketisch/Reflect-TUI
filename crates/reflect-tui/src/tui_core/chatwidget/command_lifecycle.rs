//! `ChatWidget` 的命令执行生命周期处理器。
//!
//! 本模块负责命令开始/输出/完成时的渲染，包括当前 exec 单元的分组以及统一的 exec 等待状态。
...
impl ChatWidget {
    ...
    pub(super) fn on_command_execution_started(&mut self, item: ThreadItem) {
        ...
        if is_unified_exec_source(*source) {
            ...
            // 统一 exec 可能会被解析为 Unknown；无论如何都要保持工作指示器可见。
    ...
    pub(super) fn on_terminal_interaction(&mut self, process_id: String, stdin: String) {
        ...
        self.flush_answer_stream_with_separator();
        if stdin.is_empty() {
            // 空 stdin 表示我们正在轮询后台输出。
            // 将其显示在状态指示器（统一的“等待”界面）上，而不是渲染到对话记录中。
            // 保持标题简短，以便中断提示仍然可见。
            self.bottom_pane.ensure_status_indicator();
            self.bottom_pane
    ...
    /// 为统一 exec 页脚记录最近的 stdout/stderr 行。
    ...
    /// 在保留当前 exec 单元分组契约的前提下，完成一次 exec 调用。
    ///
    /// Exec 开始/结束事件通常通过 `running_commands` 成对出现，但统一 exec 可能会为某个调用
    /// 发出结束事件，而该调用从未作为当前活跃 `ExecCell` 实体化（例如另一个正在进行的
    /// 探索组仍处于活跃状态）。在这种情况下，我们将结束事件作为独立的历史记录项渲染，
    /// 而不是替换或刷新不相关的活跃探索单元。如果此方法将每个未知的结束事件都视为
    /// “完成当前单元”，UI 可能会合并不相关的命令并隐藏仍在运行的探索任务。
    pub(crate) fn handle_command_execution_completed_now(&mut self, item: ThreadItem) {
        enum ExecEndTarget {
            // 正常情况：当前活跃 exec 单元已跟踪此调用 id。
            ActiveTracked,
            // 存在当前活跃 exec 组，但其不包含此调用 id。将结束事件作为独立的最终化
            // 历史单元渲染，以保持当前活跃组完整。
            OrphanHistoryWhileActiveExec,
            // 没有活跃 exec 单元可以安全地拥有此结束事件；基于结束事件负载创建新单元。
            NewCell,
        }
        ...
        let event_parsed = command_actions
            .into_iter()
            .map(crate::app_server_protocol::CommandAction::into_core)
            .collect();
        ...
        // 统一 exec 交互行刻意在 exec 单元中隐藏命令输出文本，而将交互相关内容
        // 渲染在 UI 的其他位置。
