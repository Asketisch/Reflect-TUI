//! 斜杠命令分发。
//!
//! 注：本文件超过 800 行红线，但 `impl ChatWidget` 斜杠命令分发方法簇为内聚的完整
//! 状态机（15 个大方法，每方法处理一个命令），与上游逐行对应，
//! 按 AGENTS.md「内聚完整状态机例外」保留。

//! `ChatWidget` 的斜杠命令分发与本地上箭头回忆交接。
//!
//! `ChatComposer` 解析斜杠输入，并在返回输入结果之前，把识别出的命令文本
//! 暂存以供本地上箭头回忆。本模块负责 app 级分发步骤，并在命令被处理后
//! 记录暂存条目，使斜杠命令回忆与普通文本遵循相同的已提交输入规则。

use super::*;
use crate::tui_core::app_event::ThreadGoalSetMode;
...
use crate::tui_core::goal_display::GOAL_USAGE;
...
const USAGE_CHATGPT_LOGIN_REQUIRED: &str = "Sign in to Reflect to use /usage.";

impl ChatWidget {
    /// 分发裸斜杠命令，并记录其暂存的本地历史条目。
    ///
    /// composer 在返回 `InputResult::Command` 前会暂存历史；此包装器在
    /// 分发后提交该暂存条目，使斜杠命令的回忆遵循与普通文本
    /// 相同的“已提交输入”规则。
    ...
    /// 分发内联斜杠命令，并记录其暂存的本地历史条目。
    ///
    /// 内联命令参数稍后可通过正常提交管线准备，但
    /// 本地命令回忆仍跟踪原始命令调用。将此包装器视为
    /// 唯一的输入结果入口点，可避免带内联参数的命令被双重记录。
        ...
        match cmd {
            SlashCommand::Feedback => {
                if !self.config.feedback_enabled {
                    let params = crate::tui_core::bottom_pane::feedback_disabled_params();
                    self.bottom_pane.show_selection_view(params);
                    self.request_redraw();
                    return;
                }
                // 第 1 步：选择分类（UI 构建于 feedback_view 中）
                let params = crate::tui_core::bottom_pane::feedback_selection_params(
            ...
            SlashCommand::Archive => {
                self.bottom_pane.show_selection_view(SelectionViewParams {
                    ...
                    subtitle: Some(
                        "Are you sure? This will archive the current session and exit Reflect"
                            .to_string(),
                    ),
            ...
            SlashCommand::Rename => {
                self.session_telemetry
                    .counter("reflect.thread.rename", /*inc*/ 1, &[]);
            ...
            SlashCommand::Loop => {
                // `/loop` 需要参数,空参仅显示用法。真正实现走 stable TUI 的
                // 文本提交路径(`SlashOutcome::Loop`),这里给出提示。
                self.add_info_message(
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.session_telemetry.counter(
                        "reflect.windows_sandbox.setup_elevated_sandbox_command",
                        /*inc*/ 1,
                        &[],
                    );
                    self.app_event_tx
    ...
    /// 运行内联斜杠命令。
    ///
    /// 准备参数的分支应给 composer 传 `record_history: false`，因为
    /// 暂存的斜杠命令条目才是回忆记录；若同时走正常提交历史
    /// 路径，单条命令会在向上键导航时出现两次。
    ...
    fn dispatch_prepared_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Keymap => match trimmed.to_ascii_lowercase().as_str() {
                "" => self.open_keymap_picker(),
                "debug" => {
                    match crate::tui_core::keymap::RuntimeKeymap::from_config(
    ...
    fn builtin_command_flags(&self) -> BuiltinCommandFlags {
        ...
        BuiltinCommandFlags {
            ...
            token_activity_command_enabled: self.has_reflect_backend_auth,
    ...
    fn ensure_usage_command_available(&mut self) -> bool {
        if self.has_reflect_backend_auth {
            return true;
        }
    ...
    fn queued_command_drain_result(&self, cmd: SlashCommand) -> QueueDrain {
        ...
        match cmd {
            ...
            | SlashCommand::Theme
            | SlashCommand::Pets
            | SlashCommand::Loop => QueueDrain::Stop,
        }
    }
