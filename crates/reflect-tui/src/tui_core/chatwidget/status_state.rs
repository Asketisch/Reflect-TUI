...
/// 可渲染进终端标题的紧凑运行时状态。
///
/// 该集合刻意比完整的状态头词汇表更小。终端标题需要简短、稳定的标签，
/// 因此调用方在渲染前会把更丰富的生命周期事件映射到这些桶之一。
...
impl PendingGuardianReviewStatus {
    ...
    // 守护审查状态由当前全部待处理审查条目推导而来。`ChatWidget` 上的通用状态缓存
    // 保存的是当前渲染的页脚；此辅助函数计算在审查进行期间应替换它的、守护专属的
    // 页脚快照。
