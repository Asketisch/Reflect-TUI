//! TUI 转录中成组执行调用历史单元的数据模型。
//!
//! 一个 `ExecCell` 既可以表示单个命令，也可以表示一个由相关 read/list/search 命令组成的
//! “浏览”分组。聊天控件依赖稳定的 `call_id` 匹配来将进度事件和结束事件路由到正确的单元，
//! 并将“找不到调用 id”视为一个真实信号（例如，应当作为单独历史条目渲染的孤立结束事件）。
...
pub(crate) struct CommandOutput {
    ...
    /// 最终的、交织在一起的 stderr 与 stdout，会替换掉任何流式预览。
...
/// 将最近一次匹配上的调用标记为已完成，并返回是否找到了对应的调用。
///
/// 调用方应当将 `false` 视为路由不匹配，而不是静默忽略。聊天控件利用这个信号来避免将
/// 孤立的 `exec_end` 事件附加到一个不相关的活跃浏览单元上，否则会把两条转录条目错误地
/// 合并在一起。
