...
const APP_TOOLTIP: &str = "Try the **Desktop app**. Run 'reflect app'.";
const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP: &str = "*New* Build faster with the **Desktop app**. Run 'reflect app'.";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with Reflect.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Reflect is included in your plan for free – let’s build together.";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("reflect app") {
                return false;
            }
            true
        })
...
/// 付费用户在大多数启动会话中看到的是专属推广位，而非通用的随机提示池。
/// 将这段业务逻辑显式化：目前该推广位在应用推广与 Fast 模式之间分配，
/// 但当用户已启用 Fast 模式时，会抑制 Fast 推广。
...
fn pick_tooltip<R: Rng + ?Sized>(mut rng: &mut R) -> Option<&'static str> {
...
pub(crate) mod announcement {
    ...
    /// 预热公告提示的缓存。
    ///
    /// Reflect 不再从远端端点获取公告提示，因此这是一个
    /// 为保持调用点兼容而保留的空操作。
    pub(crate) fn prewarm() {}

    /// 获取公告提示；若预热尚未完成则返回 None。
    ///
    /// Reflect 不再获取远端公告提示，因此这里总是返回 None。
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // Reflect 目前发布 macOS、Windows 和 Linux 的 CLI 构建。
                Self::Linux
            }
        }
    }
    ...
    pub(crate) fn parse_announcement_tip_toml(
        ...
        for raw in announcements {
            ...
            if tip.version_matches(REFLECT_CLI_VERSION)
...
mod tests {
    use super::*;
    use crate::tui_core::tooltips::announcement::parse_announcement_tip_toml;
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Reflect TUI.
...
[[announcements]]
content = "Welcome to Reflect! Check out the new onboarding flow."
