...
/// 在列中布局子元素，可为每个子元素指定 flex 因子。
///
/// flex 因子 > 0 的子元素在非 flex 子元素之后获得剩余空间，
/// 按 flex 因子比例分配。
impl<'a> FlexRenderable<'a> {
    ...
    fn allocate(&self, area: Rect) -> Vec<Rect> {
        ...
        // 2. 满足那些所需空间小于其比例份额的 flex 子项，使其未用空间
        //    可以重新分配，而不是留下空白行。
