//! 语法高亮（render/highlight） 的测试集。
//!
//! 从 render/highlight.rs 的内联 mod tests 块外移而来，业务逻辑零改动，
...
fn write_minimal_tmtheme(path: &Path) {
    // 最小有效 .tmTheme plist（足以让 syntect 解析）。
    std::fs::write(
            path,
...
fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
    let theme = resolve_theme_by_name(theme_name, /*reflect_home*/ None)
...
fn fallback_trailing_newline_no_phantom_line() {
    // pulldown-cmark 发送的代码块文本以 '\n' 结尾。
    // 回退路径（未知语言）绝不能由此换行产生虚构的尾部空行。
...
fn convert_style_suppresses_underline() {
    // Dracula（以及其他主题）会在类型作用域上设置 FontStyle::UNDERLINE，
    // 导致终端输出中类型名下出现分散注意力的下划线。
    // convert_style 必须像抑制斜体一样抑制下划线。
    let syn = SyntectStyle {
        foreground: syntect::highlighting::Color {
            r: 100,
            g: 200,
...
fn highlight_large_input_falls_back() {
    // 超过 MAX_HIGHLIGHT_BYTES 的输入应返回 None（纯文本回退），
    // 而不是尝试解析。
...
fn highlight_many_lines_falls_back() {
    // 超过 MAX_HIGHLIGHT_LINES 的输入应返回 None。
...
fn highlight_many_lines_no_trailing_newline_falls_back() {
    // 恰好 MAX_HIGHLIGHT_LINES+1 行但没有尾部换行的片段，
    // 只有 MAX_HIGHLIGHT_LINES 个换行字节。该防护必须统计实际行数，
    // 而非换行字节数，才能捕获这种情况。
...
fn bundled_theme_can_provide_diff_scope_backgrounds() {
    let theme = resolve_theme_by_name("github", /*reflect_home*/ None)
...
fn validate_theme_name_none_for_bundled() {
    // 内置主题不应产生任何警告。
    assert!(validate_theme_name(Some("dracula"), /*reflect_home*/ None).is_none());
    assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
}

#[test]
fn validate_theme_name_none_when_no_override() {
    assert!(validate_theme_name(/*name*/ None, /*reflect_home*/ None).is_none());
}
...
fn parse_theme_name_is_exhaustive() {
    ...
    // 嵌入集合中的每个变体都必须能通过 parse_theme_name 到达。
    let all_variants = EmbeddedLazyThemeSet::theme_names();

    // 防护：如果 two-face 新增了主题，本测试会强制我们更新映射。
    assert_eq!(
        all_variants.len(),
        32,
