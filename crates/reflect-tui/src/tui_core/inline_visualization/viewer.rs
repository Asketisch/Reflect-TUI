...
const VIEWER_DIRECTORY_NAME: &str = ".reflect-viewers";

// 保持这些资源与打包的 visualize 技能的浏览器渲染器一致。
...
fn render_fragment(fragment: &str, title: &str) -> String {
    ...
    // srcdoc iframe 会继承父页面的 CSP，因此外层壳必须允许较严格的 frame CSP
    // 可能使用的所有资源类型。iframe 本身仍然处于沙箱中，且未启用 allow-same-origin。
