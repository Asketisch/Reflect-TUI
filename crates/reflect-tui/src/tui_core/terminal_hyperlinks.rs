//! 与可见 TUI 文本分离携带的语义终端超链接。
//!
//! 布局代码测量并折行普通的 ratatui 行。只有当文本到达
//! 终端缓冲区或回滚写入器时才应用超链接标注，使 OSC 8 字节永不
//! 影响几何。
...
use crate::tui_core::render::line_utils::line_to_borrowed;
...
impl TerminalHyperlink {
    ...
    pub(crate) fn retarget_to_trusted_file(&mut self, destination: &Url) {
        // 把 file URL 排除在通用 Markdown 链接路径之外。只有生成的
        // 可视化链接会被提升为该目的地类型。
...
/// 可见文本折行把一行拆分后，重新挂接源超链接范围。
///
/// 链接文本按显示顺序匹配，因此跨表格行拆分的 URL 会在每个
/// 渲染片段上保留完整的
/// 目的地。匹配时忽略行边界处插入或删除的空白；超链接
/// 目的地本身绝不从输出重建。
...
mod tests {
    ...
    fn trusted_file_destination_receives_osc8_without_enabling_plain_file_links() {
        ...
        let mut link = TerminalHyperlink::web(
            /*columns*/ 0..4,
            "https://reflect.invalid/viewer".to_string(),
        );
