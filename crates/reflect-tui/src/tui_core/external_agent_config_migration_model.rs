...
pub(crate) fn external_agent_config_migration_groups(
    ...
    if !projects.is_empty() {
        ...
        groups.push(ExternalAgentConfigMigrationGroupModel {
            ...
            description: "Add Reflect files alongside your existing project files",
...
fn external_agent_config_migration_type_label(
...
/// 汇总所选迁移项表示的具体对象。
///
/// 大多数检测到的项类型把要导入的对象放在 `details` 中；没有
/// details 的类型表示每个迁移项对应一个可导入的文件或源目录。
