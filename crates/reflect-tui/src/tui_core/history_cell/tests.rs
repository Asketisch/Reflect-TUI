...
use crate::protocol_compat::mcp::CallToolResult;
use crate::protocol_compat::mcp::Tool;
use rmcp::model::Content;

const SMALL_PNG_BASE64: &str = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==";
async fn test_config() -> Config {
    let reflect_home = std::env::temp_dir();
    ConfigBuilder::default()
        .reflect_home(reflect_home.clone())
        .build()
        .await
        .expect("config")
}
...
fn test_cwd() -> PathBuf {
    // 这些测试只需要一个稳定的绝对 cwd；使用 temp_dir() 可避免将 Unix 或
    // Windows 特有的根目录语义写死在测试夹具中。
    std::env::temp_dir()
}
...
fn composite_cell_preserves_child_web_links() {
    let destination = "https://chatgpt.com/reflect/settings/usage";
    ...
    assert_eq!(
        lines[2].hyperlinks,
        vec![
            crate::tui_core::terminal_hyperlinks::TerminalHyperlink::web(
...
fn session_configured_event(model: &str) -> ThreadSessionState {
    ThreadSessionState {
        ...
        approvals_reviewer: crate::protocol_compat::config_types::ApprovalsReviewer::User,
...
fn mcp_tools_output_from_statuses_renders_status_only_servers() {
    let statuses = vec![McpServerStatus {
        ...
        auth_status: crate::app_server_protocol::McpAuthStatus::Unsupported,
    }];
...
fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
    ...
    assert_eq!(
        rendered,
        vec![
            "✔ You approved reflect to".to_string(),
...
fn coalesces_sequential_reads_within_one_call() {
    // 构建一个 exec 单元，其中先执行一次 Search，再执行两次 Read
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    // 创建一个包含多行命令的已完成 exec 单元
    let cmd = "set -o pipefail\ncargo test -p reflect-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),
...
fn stderr_tail_more_than_five_lines_snapshot() {
    // 构建一个非零退出且 stderr 包含 10 行的 exec 单元，以覆盖
    // 头尾渲染和装订线前缀。
...
fn ran_cell_multiline_with_stderr_snapshot() {
    // 构建一个已完成（因此渲染为“Ran”）的 exec 单元，其中命令
    // 足够长，必须单独渲染在标题下方，并包含几行 stderr，
    // 以验证输出块的
    // 前缀和换行。
...
fn user_history_cell_wraps_and_prefixes_each_line_snapshot() {
    ...
    // 使用较小宽度以更明显地强制换行。由于 ▌ 前缀和尾随空格，有效换行宽度为 width-2。
    let width: u16 = 12;
    let lines = cell.display_lines(width);
    let rendered = render_lines(&lines).join("\n");
...
fn plan_update_with_note_and_wrapping_snapshot() {
    // 较长的说明会强制换行；加入较长的步骤文本以验证步骤换行和对齐。
...
fn agent_markdown_cell_survives_insert_history_rewrap() {
  ...
  ";
    ...
    // 模拟 insert_history_lines 的行为：使用终端宽度且不带缩进调用
    // word_wrap_lines。
...
/// 模拟 `App::handle_event` 中向后遍历的合并逻辑，
/// 以验证它能正确识别并替换连续的 `AgentMessageCell`。
#[test]
fn consolidation_walker_replaces_agent_message_cells() {
    use std::sync::Arc;

    // 构建 transcript：[UserCell, AgentMsg(head), AgentMsg(cont), AgentMsg(cont)]
