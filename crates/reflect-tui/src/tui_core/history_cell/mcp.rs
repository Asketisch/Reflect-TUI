...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<crate::protocol_compat::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<crate::protocol_compat::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // 为树形前缀（"  └ "/"    "）预留四列，并确保包装器至少仍有一个可用单元。
        let detail_wrap_width = (width as usize).saturating_sub(4).max(1);

        if let Some(result) = &self.result {
            match result {
                Ok(crate::protocol_compat::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
/// 当 MCP 工具结果包含可解码的图像时，返回一个额外的历史单元。
///
/// 此函数有意最多返回一个单元：`CallToolResult.content` 中第一个能成功进行 base64 解码并解析为图像的项。
/// 这作为独立于主 MCP 工具调用单元的轻量级“存在图像输出”提示。
///
/// 手动测试提示：
/// - 运行 rmcp stdio 测试服务器（上游 rmcp-client test_stdio_server），
///   并通过 `reflect mcp add` 将其注册为 MCP 服务器。
/// - 使用其 `image_scenario` 工具，配合 `text_then_image`、
///   `invalid_base64_then_image` 或 `invalid_image_bytes_then_image` 等用例进行验证，
///   以确保即使第一个块不是有效图像，也能触发此路径。
...
/// 将 MCP 的 `ImageContent` 块解码为内存中的图像。
///
/// 当该块不是图像、base64 解码失败、无法推断格式，或图像解码器拒绝这些字节时，返回 `None`。
...
/// 使用完全限定的工具名称，按连接分组渲染 MCP 工具。
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, crate::protocol_compat::mcp::Tool>,
...
/// 根据 app-server 的 `McpServerStatus` 响应构建 `/mcp` 历史单元。
///
/// 服务器列表直接来自 app-server 的状态响应，按字母顺序排序。
/// TUI 刻意不使用客户端本地配置来丰富这些行，因为远程 MCP 状态由 app-server 拥有。
///
/// 此函数的布局与 [`new_mcp_tools_output`] 类似，但数据来自分页的 RPC 响应，
/// 而不是进程内的 `McpManager`。`detail` 标志控制是否渲染资源和资源模板。
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        ...
        let auth_status = match status.auth_status {
            crate::app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
            crate::app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
            crate::app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
            crate::app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
        };
        lines.push(
            vec![
                "    • Auth: ".into(),
...
/// MCP 清单 RPC 进行中时显示动画旋转器的临时历史单元。
///
/// 由 `ChatWidget::add_mcp_output()` 作为 `active_cell` 插入，并在拉取完成后移除。
/// 应用会从转录历史中移除已提交的副本，
/// 而 `ChatWidget::clear_mcp_inventory_loading()` 仅清除进行中的 `active_cell`。
#[derive(Debug)]
...
/// [`McpInventoryLoadingCell`] 的便捷构造函数。
