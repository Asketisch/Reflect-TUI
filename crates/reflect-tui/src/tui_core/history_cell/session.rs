...
/// 在边框内渲染 `lines`，边框宽度按内容中最宽的 span 确定。
pub(crate) fn with_border(lines: Vec<Line<'static>>) -> Vec<Line<'static>> {
    with_border_internal(lines, /*forced_inner_width*/ None)
}

/// 在内部宽度至少为 `inner_width` 的边框内渲染 `lines`。
///
/// 当调用方已将内容限制为特定宽度，并希望在此集中处理边框计算，
/// 而不是在 TUI widget 自身中重复
/// 内边距逻辑时，这会很有用。
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Reflect".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what Reflect is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
        ...
        if config.show_tooltips
            && let Some(tooltips) = tooltip_override
                ...
                .map(|tip| TooltipHistoryCell::new(tip, config.cwd.as_path()))
        {
    ...
    pub(crate) fn format_directory_inner(directory: &Path, max_width: Option<usize>) -> String {
        ...
        if let Some(max_width) = max_width {
            ...
            if UnicodeWidthStr::width(formatted.as_str()) > max_width {
                return crate::tui_core::text_formatting::center_truncate_path(
                    &formatted, max_width,
                );
            }
        }
...
impl HistoryCell for SessionHeaderHistoryCell {
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let mut lines = vec![
            Line::from(format!("Reflect (v{})", self.version)),
            Line::from(format!(
                "model: {}{}",
                self.model,
