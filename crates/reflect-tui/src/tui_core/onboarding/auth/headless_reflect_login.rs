...
pub(super) fn start_headless_reflect_login(widget: &mut AuthModeWidget) {
...
pub(super) fn render_device_code_login(
        ...
        (&state.verification_url, &state.user_code)
    {
        ...
        lines.push("".into());
        lines.push(
            "  Continue only if you started this login in Reflect. If a website or another person gave you this code, cancel."
                .dim()
                .into(),
        );
        lines.push("".into());
...
fn set_device_code_state_for_active_attempt(
    ...
    request_frame: &crate::tui_core::tui::FrameRequester,
    request_id: &str,
...
mod tests {
    ...
    fn set_device_code_state_for_active_attempt_updates_only_when_active() {
        let request_frame = crate::tui_core::tui::FrameRequester::test_dummy();
