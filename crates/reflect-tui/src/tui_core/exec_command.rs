...
use crate::shell_command::parse_command::extract_shell_command;
use dirs::home_dir;
use shlex::try_join;
...
pub(crate) fn strip_bash_lc_and_escape(command: &[String]) -> String {
    if let Some(script) = extract_shell_command(command) {
        return script;
    }
...
/// 如果 `path` 是绝对路径且位于 $HOME 之内，则返回主目录*之后*的部分；
/// 否则原样返回该路径。注意，如果 `path` 本身就是主目录，则会返回一个空路径。
