...
use crate::tui_core::terminal_detection::TerminalName;
use crate::tui_core::terminal_detection::terminal_info;
...
pub(crate) fn set_default_colors_from_startup_probe(
    colors: Option<crate::tui_core::terminal_probe::DefaultColors>,
) {
...
mod imp {
    ...
    pub(super) fn requery_default_colors() {
        if let Ok(mut cache) = default_colors_cache().lock() {
            ...
            // 焦点事件在 crossterm 的事件流激活之后才会到达。这里通过
            // crossterm 重新查询，使无关输入保留在 crossterm 的跳过事件队列中，
            // 而不会被有界启动探测的直接 tty 读取所消费。
    ...
    /// 通过有界启动探测路径查询终端默认颜色。
    ///
    /// 调色板缓存将 `None` 视为「已尝试但不可用」的结果，因此该函数
    /// 将 I/O 错误与缺失响应统一归入同一条回退路径，即那些根本
    /// 不支持 OSC 10/11 查询的终端所使用的回退路径。
    fn query_default_colors() -> Option<DefaultColors> {
        crate::tui_core::terminal_probe::default_colors(
            crate::tui_core::terminal_probe::DEFAULT_TIMEOUT,
        )
        .ok()
        .flatten()
...
mod imp {
    ...
    #[cfg(any(unix, windows))]
    pub(super) fn set_default_colors_from_startup_probe(
        _colors: Option<crate::tui_core::terminal_probe::DefaultColors>,
    ) {
    }
...
/// 在各终端间通常保持一致的 Xterm 颜色子集。
fn xterm_fixed_colors() -> impl Iterator<Item = (usize, (u8, u8, u8))> {
    XTERM_COLORS.into_iter().enumerate().skip(16)
}

// Xterm 颜色；来源于 https://ss64.com/bash/syntax-colors.html
pub const XTERM_COLORS: [(u8, u8, u8); 256] = [
    // 前 16 种颜色随终端主题而变化，因此使用这些索引时，
    // 实际显示的颜色很可能并非这里的值。
    (0, 0, 0),       //   0 Black (系统)
    (128, 0, 0),     //   1 Maroon (系统)
    (0, 128, 0),     //   2 Green (系统)
    (128, 128, 0),   //   3 Olive (系统)
    (0, 0, 128),     //   4 Navy (系统)
    (128, 0, 128),   //   5 Purple (系统)
    (0, 128, 128),   //   6 Teal (系统)
    (192, 192, 192), //   7 Silver (系统)
    (128, 128, 128), //   8 Grey (系统)
    (255, 0, 0),     //   9 Red (系统)
    (0, 255, 0),     //  10 Lime (系统)
    (255, 255, 0),   //  11 Yellow (系统)
    (0, 0, 255),     //  12 Blue (系统)
    (255, 0, 255),   //  13 Fuchsia (系统)
    (0, 255, 255),   //  14 Aqua (系统)
    (255, 255, 255), //  15 White (系统)
    // 其余颜色在大多数终端中保持一致。
    (0, 0, 0),       //  16 Grey0
    (0, 0, 95),      //  17 NavyBlue
    (0, 0, 135),     //  18 DarkBlue
