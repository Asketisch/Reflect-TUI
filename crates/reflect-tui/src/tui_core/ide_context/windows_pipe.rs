//! IDE 上下文 IPC 客户端的 Windows 命名管道传输层。
...
impl OverlappedOperation {
    ...
    fn complete(
        ...
        if initial_result == 0 {
            ...
            // 截止时间后使用零等待，使未决的重叠 I/O 仍经由 cancel_and_timeout
            // 流转，而不是在 OS 操作仍持有该 OVERLAPPED 时直接返回。
    ...
    fn cancel_and_timeout(&mut self, handle: HANDLE) -> io::Error {
        ...
        if cancel_result == 0 {
            ...
            // ERROR_NOT_FOUND 表示操作在发出取消前已完成。不等待直接排空它，
            // 使超时路径不会阻塞超过调用方的截止时间。
            let mut bytes_transferred = 0;
            unsafe {
                GetOverlappedResult(handle, self.as_mut_ptr(), &mut bytes_transferred, FALSE)
...
impl TokenUserBuffer {
    fn sid(&self) -> io::Result<windows_sys::Win32::Foundation::PSID> {
        ...
        // GetTokenInformation 会把 TOKEN_USER 写入字节缓冲区。Vec<u8> 不保证
        // TOKEN_USER 的对齐，因此在借助其 SID 指针之前，须用非对齐读取
        // 把固定头部复制出来。
        let token_user =
