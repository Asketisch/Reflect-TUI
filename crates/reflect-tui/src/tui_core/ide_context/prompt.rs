//! 注入到 TUI 用户回合中的 IDE 上下文的提示渲染。

use crate::app_server_protocol::ByteRange;
use crate::app_server_protocol::TextElement;
...
// 与桌面应用和 IDE 扩展的分隔符完全一致。IDE 上下文会在该标记之前
// 序列化进原始提示，随后会话渲染会裁剪回最后一个标记之后的请求。
// 保持相同的标记与裁剪语义，可让在一个界面中携带 IDE 上下文创建的线程
// 在其他界面中干净地回放。
const PROMPT_REQUEST_BEGIN: &str = "## My request for Reflect:";
